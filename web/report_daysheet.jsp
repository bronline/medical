...
<script>
    function printReport(target) {
      window.open(target,'print','height=300,width=690,scrollbars=yes,resizable');
    }
</script>
<%
    RWInputForm frm = new RWInputForm();
    ResultSet rRs=io.opnRS("select 0 as id, '*All' as name union select id, name from resources");
...
// Begin printing the report
    out.print("<div style=\" height: 300; width: 700; overflow: auto;\">");
...
//---------------------------------------------------------------------------------------------------------------------------//
    ...
    if(!resourceId.equals("0")) { myQuery += "and c.resourceId=" + resourceId + " "; }

    myQuery += "order by 2,1, 3";
    ...
    if (pRs.next()) {
        do {
            ...
            // Print the table headings
                ...
                out.print("<tr>");
                out.print("<th style=\"border-left:1px solid black;\" width=50>Date</td>");
                out.print("<th width=300>Description</td>");
                out.print("<th width=150>Patient</td>");
                out.print("<th width=100 style=\"border-right:1px solid black;\">Amount</td>");
                out.print("</tr>");
            }
...
//---------------------------------------------------------------------------------------------------------------------------//
    myQuery = "SELECT b.date, c.description, concat(lastname,', ', firstname) as patient " +
                ", a.quantity*a.chargeamount as amount, 'charge' as chargetype " +
                "from charges a join " +
    ...
    if(!resourceId.equals("0")) { myQuery += "and a.resourceId=" + resourceId + " "; }

    myQuery += "order by chargetype, date, patient";
